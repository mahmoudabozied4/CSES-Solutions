#include <bits/stdc++.h>
// علمكم الله ما ينفعكم و نفعكم بما تعلمتم و زادكم علما
// وَأَن لَّيْسَ لِلْإِنسَانِ إِلَّا مَا سَعَىٰ، وَأَنَّ سَعْيَهُ سَوْفَ يُرَىٰ
using namespace std;
#define ll long long
#define int long long
#define ull unsigned long long
#define endl '\n'
typedef pair<int, int> pii;
#define elif else if
#define endd(s) return void(cout << s << "\n")
#define Ceil(n, m) (((n) / (m)) + ((n) % (m) ? 1 : 0))
#define X first
#define Y second
#define fixed(n) fixed << setprecision(n)
#define sz(s) int32_t(s.size())
#define dbg(x) cout << #x << ": " << (x) << "\n";
#define getline(s) getline(cin >> ws, s)
#define Time cerr << "Time Taken: " << (float)clock() / CLOCKS_PER_SEC << " Secs" << "\n";
#define all(vec) vec.begin(), vec.end()
#define rall(v) v.rbegin(),v.rend()
#define mul_mod(a, b) (((a % M) * (b % M)) % M)
#define add_mod(a, b) (((a % M) + (b % M)) % M)
const int N = 2e4, M = 1e9 + 7, OO = 0X3F3F3F3F3F3F3F3F;
const double EPS = 1e-9, pi = 3.141592653589793;
#define kill return 0
vector<string> RET = {"NO", "YES"};
#define IO(NAME) \
cin.tie(0)->sync_with_stdio(0); \
if(fopen(NAME ".in","r")) freopen(NAME ".in","r",stdin), \
freopen(NAME ".out","w",stdout);
 
void Zied() {
    ios_base::sync_with_stdio(false), cin.tie(nullptr), cout.tie(nullptr);
#ifndef ONLINE_JUDGE
    freopen("input.txt", "r", stdin), freopen("output.txt", "w", stdout);
#endif
}
 
template<typename T = int>
istream &operator >>(istream &in, vector<T> &v) {
    for (auto &x: v) in >> x;
    return in;
}
 
template<typename T = int>
ostream &operator <<(ostream &out, const vector<T> &v) {
    for (const T &x: v) out << x << ' ';
    return out;
}
 
enum dir { d, r, u, l, ul, dr, ur, dl };
 
int dx[8] = {1, 0, -1, 0, -1, 1, -1, 1};
int dy[8] = {0, 1, 0, -1, -1, 1, 1, -1};
 
 
void preprocessing() {
}
 
vector<vector<int> > adj;
vector<int> tin, tout, parent, depth, sum, vals;
int timer;
 
void dfs(int node,int P,int d) {
    tin[node] = timer;
    timer++;
    parent[node] = P;
    depth[node] = d;
 
    for (auto v: adj[node]) {
        if (v == P)continue;
        dfs(v, node, d + 1);
    }
    tout[node] = timer;
    timer++;
}
 
bool is_ancestor(int u,int v) {
    return (u == 0 || (tin[u] <= tin[v] && tout[v] <= tout[u]));
}
 
 
vector<vector<int> > ancestors;
vector<vector<int> > mn;
 
void build(int n) {
    for (int v = 1; v <= n; v++) {
        ancestors[v][0] = parent[v];
        mn[v][0] = min(v, parent[v]);
    }
    for (int i = 1; i < 20; i++) {
 
        for (int v = 1; v <= n; v++) {
            int tmp = ancestors[v][i - 1];
            ancestors[v][i] = ancestors[tmp][i - 1];
            mn[v][i] = min(mn[v][i - 1], mn[tmp][i - 1]);
        }
    }
}
 
int lca(int u,int v) {
    if (is_ancestor(u, v))return u;
    for (int i = 19; i >= 0; i--) {
        if (!is_ancestor(ancestors[u][i], v))u = ancestors[u][i];
    }
    return ancestors[u][0];
}
 
int kth_ancestor(int v,int k){
    if(k==0)return v;
    for(int i=0;i<20;i++){
        if((k&(1<<i)))v=ancestors[v][i];
    }
    return v;
}
 
int distance(int u,int v) {
    int LCA = lca(u, v);
    return depth[u] + depth[v] - 2 * depth[LCA];
}
 
 
void Solve() {
    int n , q;
    cin >> n >> q;
 
    adj = vector<vector<int> >(n + 1, vector<int>());
    tin = vector<int>(n + 1, 0);
    tout = vector<int>(n + 1, 0);
    parent = vector<int>(n + 1, 0);
    ancestors = vector<vector<int> >(n + 1, vector<int>(20, 0));
    mn = vector<vector<int> >(n + 1, vector<int>(20, 0));
    depth = vector<int>(n + 1, 0);
    sum = vector<int>(n + 1, 0);
    timer = 0;
 
 
    vector<int> v(n - 1) ; cin >> v;
 
    for (int i = 2; i <=n; i++) {
        adj[i].push_back(v[i - 2]);
        adj[v[i - 2]].push_back(i);
    }
 
 
    dfs(1, 0, 0);
    build(n);
 
    while (q--) {
        int u, k;cin >> u >> k ;
        if (k > depth[u]) cout << -1 << endl;
        else cout << kth_ancestor(u, k) << endl;
    }
}
 
int32_t main() {
    Zied();
    preprocessing();
    int test_cases = 1;
    // cin >> test_cases;
    for (int tc = 1; tc <= test_cases; tc++) {
        //        cout << "Case #" << tc << ": ";
        Solve();
        //                cout << endl;
    }
    kill;
    //    Time
}
 
/*
              ,   .-'"'=;_  ,
              |\.'-~`-.`-`;/|
              \.` '.'~-.` './
              (\`,__=-'__,'/)
           _.-'-.( d\_/b ).-'-._
         /'.-'   ' .---. '   '-.`\
       /'  .' (=    (_)    =) '.  `\
      /'  .',  `-.__.-.__.-'  ,'.  `\
     (     .'.   V       V  ; '.     )
     (    |::  `-,__.-.__,-'  ::|    )
     |   /|`:.               .:'|\   |
     |  / | `:.              :' |`\  |
     | |  (  :.             .:  )  | |
     | |   ( `:.            :' )   | |
     | |    \ :.           .: /    | |
     | |     \`:.         .:'/     | |
     ) (      `\`:.     .:'/'      ) (
     (  `)_     ) `:._.:' (     _(`  )
     \  ' _)  .'           `.  (_ `  /
      \  '_) /   .'"```"'.   \ (_`  /
       `'"`  \  (         )  /  `"'`
   ___   MZ   `.`.       .'.'        ___
 .`   ``"""'''--`_)     (_'--'''"""``   `.
(_(_(___...--'"'`         `'"'--...___)_)_)
*/