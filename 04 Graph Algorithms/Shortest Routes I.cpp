...
void dijkstra(int src,vector<vector<pair<int,int>>> &adj) {
    vector<int> d(adj.size(), OO);
    vector<int> parents(adj.size(), -1);
    set<pair<int, int>> st = {{0, src}};
    d[src] = 0;
    while (!st.empty()) {
        auto x = *st.begin();
        int node = x.sc, cost = x.fr;
        st.erase(st.begin());
        if (cost != d[node])continue;
        for (auto it: adj[node]) {
            int child = it.fr, dis = it.sc;
            if (d[child] > d[node] + dis) {
                parents[child] = node;
                d[child] = d[node] + dis;
                st.insert({d[child], child});
            }
        }
    }
//    cout<<"Costs to all nodes:\n";
    for(int i=0;i<(int)adj.size();i++) {
        cout<<d[i]<<" ";
    }cout<<"\n";
//    if (d[sz(adj) - 1] == OO) cout << -1 << endl;
//    else {
//        int node = sz(adj) - 1;
//        vector<int> path;
//        while (node != -1) {
...
//        }
//        reverse(all(path));
////        cout << "Cost of the path between 0 and 3 is: " << d[3] << "\n";
//        for (auto v: path) {
//            cout << v << " ";
//        }
//        cout << "\n";
...
void Solve() {
    ...
    for (int i = 0; i < m; ++i) {
        int u , v , x ; cin >> u >> v >> x;
        u-- , v-- ;
        grid[u].push_back({v,x});
//        grid[v].push_back({u,x});
    }
    dijkstra(0 , grid);
}
