...
node merge(const node &a, const node &b) {
    return{
        max(a.pre, b.pre + a.sum), max(b.suf, b.sum + a.suf), a.sum+ b.sum,
...
node query(int qs,int qe,int ni = 0,int ns = 0,int ne = N - 1) {
    if (qs > ne || qe < ns) return {(int) -1e9, (int) -1e9, 0, (int) -1e9} ;// All Out ( Put thing Doesn't Change)
